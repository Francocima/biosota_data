...
async function main() {
  try {
    ...
    // LIMIT FOR TESTING
        if (CONFIG.testingLimit && Number(CONFIG.testingLimit) > 0) {
        console.log(`⚠ Using testing limit: only processing ${CONFIG.testingLimit} orders`);
        ordersRaw.splice(CONFIG.testingLimit); 
        }
